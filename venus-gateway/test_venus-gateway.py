...
class Test_venus_gateway_miner():
    ...
    def test_venus_gateway_miner_list(self):
        try:
            gateway_miner_list = os.popen("/root/venus-gateway miner list").readlines()
    ...
    def test_venus_gateway_miner_state(self):
        gateway_miner_list = os.popen("/root/venus-gateway miner list").readlines()[0]
        try:
            gateway_miner_state = os.popen(f"/root/venus-gateway miner state {gateway_miner_list}").readlines()
        ...
        def test_venus_gateway_wallet_list(self):
            try:
                gateway_wallet_list = os.popen("/root/venus-gateway wallet list").readlines()
        ...
        def test_venus_gateway_wallet_state(self):
            gateway_wallet_list = os.popen("/root/venus-gateway wallet list").readlines()
            wallet_for_state=gateway_wallet_list[2].split('"')[3]
            try:
                gateway_wallet_state = os.popen(f"/root/venus-gateway wallet state {wallet_for_state}").readlines()
